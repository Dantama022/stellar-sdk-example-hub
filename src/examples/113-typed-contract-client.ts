...
const BUNDLED_WASM_PATH = path.join(__dirname, '../contracts/sample-v1/upgradeable_v1.wasm');
...
export function normalizeContractId(value: string): string {
  ...
  if (!StrKey.isValidContract(trimmed)) {
    throw new Error(`Invalid Soroban contract ID "${trimmed}".`);
  }
...
function loadBundledWasm(): Buffer {
  if (!fs.existsSync(BUNDLED_WASM_PATH)) {
    throw new Error(`Bundled contract WASM was not found at ${BUNDLED_WASM_PATH}`);
  }

  const wasm = fs.readFileSync(BUNDLED_WASM_PATH);

  if (wasm.length <= 8) {
    throw new Error('Bundled WASM is too small to be a usable Soroban contract.');
  }
...
async function pollTransactionStatusRaw(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    const transaction = await rawRpcCall<RawTransactionStatus>(rpcUrl, 'getTransaction', {
      hash,
    });
    ...
    if (transaction.status === 'FAILED') {
      throw new Error(`Transaction ${hash} failed on ledger ${transaction.ledger ?? 'unknown'}.`);
    }

    if (transaction.status !== 'NOT_FOUND') {
      throw new Error(`Transaction ${hash} returned unexpected status "${transaction.status}".`);
    }
...
async function submitSorobanOperation(
  ...
  console.log(chalk.gray(`  Submitted transaction: ${submission.hash}`));

  const confirmation = await pollTransactionStatusRaw(rpcUrl, submission.hash);

  console.log(
    chalk.gray(
      `  Confirmed on ledger: ${confirmation.ledger ?? confirmation.latestLedger ?? 'unknown'}`,
    ),
  );
...
function contractIdFromSimulation(simulation: rpc.Api.SimulateTransactionSuccessResponse): string {
  ...
  if (!returnValue) {
    throw new Error('Contract deployment simulation did not return a contract address.');
  }
...
async function deployBundledContract(
  ...
  console.log('  Funding temporary Testnet account...');

  await fundAccount(deployer);

  console.log(chalk.green('  Temporary deployer funded.'));

  const wasm = loadBundledWasm();

  const wasmHash = computeWasmHash(wasm);

  console.log(`  Bundled WASM : ${BUNDLED_WASM_PATH}`);

  console.log(`  WASM size    : ${wasm.length.toLocaleString()} bytes`);
  ...
  console.log(chalk.green('WASM installation confirmed.'));
  ...
  const deploymentSimulation = await submitSorobanOperation(
    server,
    rpcUrl,
    deployer,
    ...
    Operation.createCustomContract({
      ...
      constructorArgs: [Address.fromString(deployer.publicKey()).toScVal()],
    }),
  );

  const contractId = contractIdFromSimulation(deploymentSimulation);

  console.log(chalk.green('Contract deployment confirmed.'));
...
export function resolveTypeName(typeDef: xdr.ScSpecTypeDef): string {
  ...
  if (value === xdr.ScSpecType.scSpecTypeVoid().value) {
    return 'void';
  }

  if (value === xdr.ScSpecType.scSpecTypeError().value) {
    return 'error';
  }

  if (value === xdr.ScSpecType.scSpecTypeU32().value) {
    return 'u32';
  }

  if (value === xdr.ScSpecType.scSpecTypeI32().value) {
    return 'i32';
  }

  if (value === xdr.ScSpecType.scSpecTypeU64().value) {
    return 'u64';
  }

  if (value === xdr.ScSpecType.scSpecTypeI64().value) {
    return 'i64';
  }

  if (value === xdr.ScSpecType.scSpecTypeTimepoint().value) {
    return 'timepoint';
  }

  if (value === xdr.ScSpecType.scSpecTypeDuration().value) {
    return 'duration';
  }

  if (value === xdr.ScSpecType.scSpecTypeU128().value) {
    return 'u128';
  }

  if (value === xdr.ScSpecType.scSpecTypeI128().value) {
    return 'i128';
  }

  if (value === xdr.ScSpecType.scSpecTypeU256().value) {
    return 'u256';
  }

  if (value === xdr.ScSpecType.scSpecTypeI256().value) {
    return 'i256';
  }

  if (value === xdr.ScSpecType.scSpecTypeBytes().value) {
    return 'bytes';
  }

  if (value === xdr.ScSpecType.scSpecTypeString().value) {
    return 'string';
  }

  if (value === xdr.ScSpecType.scSpecTypeSymbol().value) {
    return 'symbol';
  }

  if (value === xdr.ScSpecType.scSpecTypeAddress().value) {
    return 'address';
  }
  ...
  if (value === xdr.ScSpecType.scSpecTypeResult().value) {
  ...
  if (value === xdr.ScSpecType.scSpecTypeBytesN().value) {
 ...
 */
export function parseFunction(fn: xdr.ScSpecFunctionV0): ParsedFunction {
  return {
 ...
 */
export function parseCustomTypes(spec: contract.Spec): ParsedCustomType[] {
  const customTypes: ParsedCustomType[] = [];

  for (const entry of spec.entries) {
    const kind = entry.switch().value;

    if (kind === xdr.ScSpecEntryKind.scSpecEntryUdtStructV0().value) {
      customTypes.push({
        kind: 'struct',

        name: entry.udtStructV0().name().toString(),
      });

      continue;
    }

    if (kind === xdr.ScSpecEntryKind.scSpecEntryUdtEnumV0().value) {
      customTypes.push({
        kind: 'enum',

        name: entry.udtEnumV0().name().toString(),
      });

      continue;
    }

    if (kind === xdr.ScSpecEntryKind.scSpecEntryUdtUnionV0().value) {
      customTypes.push({
        kind: 'union',

        name: entry.udtUnionV0().name().toString(),
      });

      continue;
    }

    if (kind === xdr.ScSpecEntryKind.scSpecEntryUdtErrorEnumV0().value) {
      customTypes.push({
        kind: 'contract-error enum',

        name: entry.udtErrorEnumV0().name().toString(),
      });
    }
  }
 ...
 */
export function formatMethodSignature(fn: ParsedFunction): string {
  const args = fn.inputs.map((input) => `${input.name}: ${input.type}`).join(', ');

  const output = fn.outputs.length > 0 ? fn.outputs.join(', ') : 'void';
...
export function toTypeScriptType(type: string): string {
  ...
  if (type.startsWith('Vec<') && type.endsWith('>')) {
    return `Array<${toTypeScriptType(type.slice(4, -1))}>`;
  }

  if (type.startsWith('Option<') && type.endsWith('>')) {
    return `${toTypeScriptType(type.slice(7, -1))} | undefined`;
  }

  if (type.startsWith('BytesN<')) {
    return 'Buffer';
  }
 ...
 */
export function buildTypedInterface(functions: ParsedFunction[]): string {
  const lines: string[] = ['interface GeneratedContractClient extends contract.Client {'];
  ...
  for (const fn of functions) {
    ...
    if (fn.outputs.length === 1) {
      resultType = toTypeScriptType(fn.outputs[0]);
    } else if (fn.outputs.length > 1) {
      resultType = `[${fn.outputs.map(toTypeScriptType).join(', ')}]`;
    }
    ...
    for (const input of fn.inputs) {
      lines.push(`      ${input.name}: ${toTypeScriptType(input.type)};`);
    }

    lines.push('    },');

    lines.push('    options?: contract.MethodOptions,');

    lines.push(`  ): Promise<contract.AssembledTransaction<${resultType}>>;`);
  }
 ...
 */
function sampleValueForType(typeDef: xdr.ScSpecTypeDef, address: string): unknown {
  const type = typeDef.switch().value;

  if (type === xdr.ScSpecType.scSpecTypeAddress().value) {
    return address;
  }

  if (type === xdr.ScSpecType.scSpecTypeString().value) {
    return 'typed-client';
  }

  if (type === xdr.ScSpecType.scSpecTypeSymbol().value) {
    return 'typed_demo';
  }

  if (type === xdr.ScSpecType.scSpecTypeU32().value) {
    return 7;
  }

  if (type === xdr.ScSpecType.scSpecTypeI32().value) {
    return -7;
  }

  if (
    type === xdr.ScSpecType.scSpecTypeU64().value ||
    type === xdr.ScSpecType.scSpecTypeU128().value ||
    type === xdr.ScSpecType.scSpecTypeU256().value
  ) {
    return 7n;
  }

  if (
    type === xdr.ScSpecType.scSpecTypeI64().value ||
    type === xdr.ScSpecType.scSpecTypeI128().value ||
    type === xdr.ScSpecType.scSpecTypeI256().value
  ) {
    return -7n;
  }

  if (type === xdr.ScSpecType.scSpecTypeBytes().value) {
    return Buffer.from('typed-client', 'utf8');
  }

  if (type === xdr.ScSpecType.scSpecTypeBytesN().value) {
    return Buffer.alloc(typeDef.bytesN().n(), 0xab);
  }

  throw new Error(`No generic demonstration value is configured for ${resolveTypeName(typeDef)}.`);
}
 ...
 */
function findTypedArgumentFunction(spec: contract.Spec): xdr.ScSpecFunctionV0 | null {
  const functions = spec.funcs();

  const upgrade = functions.find(
    (fn) => fn.name().toString() === 'upgrade' && fn.inputs().length === 1,
  );
  ...
  return (
    functions.find((fn) => fn.name().toString() !== '__constructor' && fn.inputs().length === 1) ??
    null
  );
}
...
async function demonstrateGeneratedMethod(
  ...
  const selected =
    functions.find((fn) => fn.name === 'version' && fn.inputs.length === 0) ||
    functions.find((fn) => fn.name !== '__constructor' && fn.inputs.length === 0);
  ...
  const dynamicClient = client as unknown as Record<
    string,
    (options?: contract.MethodOptions) => Promise<contract.AssembledTransaction<unknown>>
  >;

  const method = dynamicClient[selected.name];

  if (typeof method !== 'function') {
    throw new Error(`Generated client method "${selected.name}" was not found.`);
  }

  const assembled = await method.call(client);

  console.log(`  Contract method : ${selected.name}`);

  console.log('  Arguments       : none');

  console.log(
    `  Return type     : ${selected.outputs.length > 0 ? selected.outputs.join(', ') : 'void'}`,
  );

  console.log(`  Decoded return  : ${displayValue(assembled.result)}`);

  console.log(
    `  JavaScript type : ${Array.isArray(assembled.result) ? 'array' : typeof assembled.result}`,
  );

  console.log(chalk.green('  Generated client invocation simulated successfully.'));
}

/**
 * Run Example 113.
 */
export async function run(params: TypedContractClientParams = {}): Promise<void> {
  console.log(chalk.bold('Soroban Typed Contract Client Generation Example'));
  ...
  let demonstrationAddress = Keypair.random().publicKey();

  if (suppliedContractId) {
    try {
      contractId = normalizeContractId(suppliedContractId);
    } catch (error: unknown) {
      console.error(chalk.red(errorMessage(error)));

      console.log(chalk.gray('Use a valid StrKey-encoded Soroban contract ID beginning with "C".'));
    ...
    console.log('  Source      : user supplied');
  } else {
    try {
      const deployment = await deployBundledContract(server, rpcUrl);

      contractId = deployment.contractId;

      demonstrationAddress = deployment.deployer.publicKey();
    } catch (error: unknown) {
      console.error(
        chalk.red(`Could not create self-contained demonstration contract: ${errorMessage(error)}`),
      );
  ...
  let deployedWasm: Buffer;

  try {
    deployedWasm = await server.getContractWasmByContractId(contractId);
    ...
    console.error(chalk.red('Could not retrieve deployed contract WASM.'));
  ...
  try {
    client = await contract.Client.fromWasm(deployedWasm, {
      contractId,
    ...
    console.error(chalk.yellow('The contract does not contain a usable Soroban specification.'));
  ...
  if (!spec || spec.entries.length === 0) {
    console.log(chalk.yellow('Contract specification is empty.'));

    return;
  }

  console.log(chalk.green(`Contract specification loaded (${spec.entries.length} entries).`));
  ...
  console.log(`  Available methods : ${functions.length}`);

  if (functions.length === 0) {
    console.log(chalk.yellow('No callable functions were found in the contract specification.'));

    return;
  }

  for (const fn of functions) {
    console.log(`\n  ${chalk.cyan(formatMethodSignature(fn))}`);

    if (fn.inputs.length === 0) {
      console.log(chalk.gray('    Argument types : none'));
    } else {
      console.log(chalk.gray('    Argument types :'));

      for (const input of fn.inputs) {
        console.log(chalk.gray(`      - ${input.name}: ${input.type}`));
      }
    }

    console.log(
      chalk.gray(
        `    Return type${fn.outputs.length === 1 ? '' : 's'}   : ${
          fn.outputs.length > 0 ? fn.outputs.join(', ') : 'void'
        }`,
      ),
    );

    if (fn.documentation) {
      console.log(chalk.gray(`    Documentation : ${fn.documentation}`));
    }
  }
  ...
  const customTypes = parseCustomTypes(spec);

  if (customTypes.length === 0) {
    console.log(
      chalk.gray('  No custom structs, enums, unions, or contract-error enums are declared.'),
    );
  } else {
  ...
  console.log(buildTypedInterface(functions));
  ...
  const generatedMethods = functions
    .filter((fn) => fn.name !== '__constructor')
    .filter((fn) => typeof (client as unknown as Record<string, unknown>)[fn.name] === 'function')
    .map((fn) => fn.name);

  console.log(
    `  Generated methods: ${generatedMethods.length > 0 ? generatedMethods.join(', ') : '(none)'}`,
  );

  if (generatedMethods.length === 0) {
    console.log(chalk.yellow('No generated client methods were available.'));
  ...
  const typedFunction = findTypedArgumentFunction(spec);
  ...
  if (!typedFunction) {
    ...
    const functionName = typedFunction.name().toString();

    const input = typedFunction.inputs()[0];

    const argumentName = input.name().toString();

    const argumentType = resolveTypeName(input.type());

    try {
      const nativeValue = sampleValueForType(input.type(), demonstrationAddress);

      const typedArguments: Record<string, unknown> = {
        [argumentName]: nativeValue,
      };

      const encoded = spec.funcArgsToScVals(functionName, typedArguments);

      console.log(`  Contract method : ${functionName}`);

      console.log(`  Argument        : ${argumentName}`);

      console.log(`  Argument type   : ${argumentType}`);

      console.log(`  Native JS value : ${displayValue(nativeValue)}`);

      console.log(`  Encoded ScVals  : ${encoded.length}`);

      encoded.forEach((scVal, index) => {
        console.log(`    [${index}] ${scVal.switch().name}`);
      });
      ...
      console.log(chalk.yellow(`Typed argument demonstration skipped: ${errorMessage(error)}`));
    }
  }
  ...
  try {
    await demonstrateGeneratedMethod(client, functions);
  } catch (error: unknown) {
    console.error(chalk.red(`Generated method simulation failed: ${errorMessage(error)}`));
