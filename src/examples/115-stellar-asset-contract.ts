...
const DEFAULT_ASSET_CODE = 'DEMO';

const DEMONSTRATION_AMOUNT = 10_000_000n;
 ...
 */
export function createIssuedAsset(assetCode: string, issuer: string): Asset {
  ...
  try {
    Keypair.fromPublicKey(issuerAddress);
  } catch {
    throw new Error(
  ...
  try {
    return new Asset(code, issuerAddress);
  } catch (error: unknown) {
    throw new Error(`Invalid Stellar asset configuration: ${errorMessage(error)}`);
  }
}
 ...
 */
export function deriveSacContractId(asset: Asset): string {
...
async function submitSimulation(
  ...
  if (submitted.status === 'ERROR') {
    throw new Error('Transaction was rejected during submission.');
  }

  const confirmation = await pollTransactionStatusRaw(rpcUrl, submitted.hash);

  if (confirmation.status !== 'SUCCESS') {
    throw new Error(`Transaction ${submitted.hash} finished with status ${confirmation.status}.`);
  }
...
async function deploySac(
  ...
  const operation = Operation.createStellarAssetContract({
    asset,
  });

  console.log(chalk.yellow('\nSimulating SAC deployment...'));

  const simulated = await simulateOperation(server, deployer, operation);

  console.log(chalk.green('SAC deployment simulation succeeded.'));

  console.log(`  Minimum resource fee : ${simulated.simulation.minResourceFee}`);

  const returnValue = simulated.simulation.result?.retval;

  let simulationContractId: string | null = null;

  if (returnValue) {
    try {
      simulationContractId = Address.fromScVal(returnValue).toString();

      console.log(`  Simulation result    : ${simulationContractId}`);
    } catch {
      const xdrValue = returnValue.toXDR('base64');

      console.log(
        `  Simulation result    : ${
          xdrValue.length > 100 ? `${xdrValue.slice(0, 100)}...` : xdrValue
        }`,
      );
    }
  } else {
    console.log('  Simulation result    : no return value');
  }

  if (simulationContractId && simulationContractId !== expectedContractId) {
    throw new Error(
  ...
  const submitted = await submitSimulation(server, rpcUrl, deployer, simulated);

  console.log(chalk.green('SAC deployment confirmed.'));

  console.log(`  Deployment tx        : ${submitted.transactionHash}`);

  console.log(
    `  Confirmed ledger     : ${
      submitted.confirmation.ledger ?? submitted.confirmation.latestLedger ?? 'unknown'
    }`,
  );

  return simulationContractId || expectedContractId;
}
 ...
 */
export async function inspectSac(server: rpc.Server, contractId: string): Promise<SacInspection> {
  const contract = new Contract(contractId);

  const response = await server.getLedgerEntries(contract.getFootprint());

  if (response.entries.length === 0) {
    throw new Error(`No contract-instance ledger entry was found for ${contractId}.`);
  }
  ...
  if (instanceValue.switch().name !== 'scvContractInstance') {
    throw new Error(`Ledger entry for ${contractId} is not a contract instance.`);
  }
  ...
  return {
    ...
    isStellarAssetContract: executableType === 'contractExecutableStellarAsset',

    // SDK 13.x types these fields as optional.
    lastModifiedLedgerSeq: ledgerEntry.lastModifiedLedgerSeq ?? null,

    liveUntilLedgerSeq: ledgerEntry.liveUntilLedgerSeq ?? null,
  };
}
 ...
 */
function displayArguments(args: xdr.ScVal[]): string {
  if (args.length === 0) {
    return 'none';
  }

  return args
    .map((arg, index) => `[${index}] ${arg.switch().name}=${displayValue(decodeScVal(arg))}`)
    .join('; ');
}
...
export async function simulateSacCall(
  ...
  method: string,
  args: xdr.ScVal[] = [],
): Promise<SimulatedSacCall> {
  const operation = contract.call(method, ...args);

  const simulated = await simulateOperation(server, source, operation);

  const returnValue = simulated.simulation.result?.retval ?? null;

  const decodedReturn = returnValue ? decodeScVal(returnValue) : null;
 ...
 */
function displayCall(call: SimulatedSacCall): void {
  console.log(`  Contract method     : ${call.method}`);

  console.log(`  Arguments           : ${displayArguments(call.args)}`);

  console.log('  Simulation result   : SUCCESS');

  console.log(`  Minimum resource fee: ${call.simulation.minResourceFee}`);

  if (call.returnValue) {
    console.log(`  Raw ScVal type      : ${call.returnValue.switch().name}`);

    const raw = call.returnValue.toXDR('base64');

    console.log(`  Raw ScVal XDR       : ${raw.length > 100 ? `${raw.slice(0, 100)}...` : raw}`);
  } else {
    console.log('  Raw ScVal           : none');
  }

  console.log(`  Decoded return      : ${displayValue(call.decodedReturn)}`);
}

/**
 * Display authorization entries returned by simulation.
 */
function displayAuthorization(call: SimulatedSacCall, sourcePublicKey: string): void {
  const authEntries = call.simulation.result?.auth ?? [];

  console.log(`  Authorization count : ${authEntries.length}`);

  if (authEntries.length === 0) {
    console.log(chalk.gray('  No explicit authorization entries were returned.'));
  ...
  authEntries.forEach((entry, index) => {
    ...
    if (credentialType === 'sorobanCredentialsSourceAccount') {
      signer = sourcePublicKey;
    } else if (credentialType === 'sorobanCredentialsAddress') {
      signer = Address.fromScAddress(credentials.address().address()).toString();
    }

    console.log(`    [${index}] ${credentialType}`);

    console.log(`        Required signer: ${signer}`);
  });
}
...
async function demonstrateUnsupportedOperation(
  ...
  const unsupportedMethod = 'total_supply';

  console.log(`  Requested method : ${unsupportedMethod}`);

  try {
    await simulateSacCall(server, source, contract, unsupportedMethod);

    console.log(chalk.yellow('  The method unexpectedly simulated successfully.'));
  } catch (error: unknown) {
    console.log(chalk.green('  Unsupported SAC operation handled gracefully.'));
 ...
 * Run Example 115.
 */
export async function run(params: StellarAssetContractParams = {}): Promise<void> {
  console.log(chalk.bold('Stellar Asset Contract (SAC) Interaction Example'));
  ...
  const generatedIssuer = params.issuer ? null : Keypair.random();

  const issuer = params.issuer?.trim() || generatedIssuer!.publicKey();

  const assetCode = (params.assetCode || process.env.ASSET_CODE || DEFAULT_ASSET_CODE)
    .trim()
    .toUpperCase();
  ...
  try {
    asset = createIssuedAsset(assetCode, issuer);
  } catch (error: unknown) {
  ...
  const sacContractId = deriveSacContractId(asset);

  console.log(`  Asset code     : ${asset.getCode()}`);

  console.log(`  Asset issuer   : ${asset.getIssuer()}`);

  console.log(`  Asset type     : ${asset.getAssetType()}`);
  ...
  const deployer = generatedIssuer || Keypair.random();

  try {
    await fundAccount(
      deployer,

      generatedIssuer ? 'Issuer/deployer' : 'SAC deployer',
    );
  } catch (error: unknown) {
    console.error(chalk.red(`Could not prepare deployment account: ${errorMessage(error)}`));
  ...
  try {
    deployedContractId = await deploySac(server, rpcUrl, deployer, asset, sacContractId);
  } catch (error: unknown) {
    console.error(chalk.red(`SAC deployment failed: ${errorMessage(error)}`));

    return;
  }

  console.log(`  Derived SAC ID       : ${sacContractId}`);

  console.log(`  Deployed SAC ID      : ${deployedContractId}`);

  console.log(`  Identification match : ${deployedContractId === sacContractId ? 'YES' : 'NO'}`);

  if (deployedContractId !== sacContractId) {
    console.error(chalk.red('The deployed contract does not match the deterministic SAC ID.'));
  ...
  let inspection: SacInspection;

  try {
    inspection = await inspectSac(server, sacContractId);
  } catch (error: unknown) {
    console.error(chalk.red(`Could not inspect SAC contract instance: ${errorMessage(error)}`));
  ...
  console.log(`  Executable type      : ${inspection.executableType}`);

  console.log(`  Built-in SAC         : ${inspection.isStellarAssetContract ? 'YES' : 'NO'}`);

  console.log(`  Last modified ledger : ${inspection.lastModifiedLedgerSeq ?? 'not reported'}`);

  console.log(`  Live until ledger    : ${inspection.liveUntilLedgerSeq ?? 'not reported'}`);

  if (!inspection.isStellarAssetContract) {
    console.error(
      chalk.red(
  ...
  console.log(chalk.green('SAC inspection verified the built-in Stellar Asset executable.'));
  ...
  const readMethods = ['name', 'symbol', 'decimals', 'admin'];

  for (const method of readMethods) {
    console.log(chalk.cyan(`\n${method}()`));

    try {
      const call = await simulateSacCall(server, deployer, sacContract, method);

      displayCall(call);
    } catch (error: unknown) {
      console.log(chalk.yellow(`  ${method}() is unavailable for this SAC configuration.`));
  ...
  const balanceAddress = Asset.native().contractId(NETWORK_PASSPHRASE);

  const balanceArgs = [Address.fromString(balanceAddress).toScVal()];

  console.log(`  Balance address : ${balanceAddress}`);

  try {
    const balanceCall = await simulateSacCall(
      server,
      deployer,
      sacContract,
      'balance',
    ...
    displayCall(balanceCall);
  } catch (error: unknown) {
    console.error(chalk.red(`Could not read SAC balance: ${errorMessage(error)}`));
  ...
  const mintArgs = [
    Address.fromString(balanceAddress).toScVal(),

    nativeToScVal(DEMONSTRATION_AMOUNT, {
      type: 'i128',
    }),
  ];
  ...
  console.log('  Contract method : mint');

  console.log(`  Recipient       : ${balanceAddress}`);

  console.log(`  Amount          : ${DEMONSTRATION_AMOUNT.toString()} base units`);

  try {
    const mintCall = await simulateSacCall(server, deployer, sacContract, 'mint', mintArgs);

    displayCall(mintCall);

    console.log('\n  Authorization requirements:');

    displayAuthorization(mintCall, deployer.publicKey());

    console.log(chalk.green('\n  Mint invocation simulation succeeded.'));

    console.log(
      chalk.yellow('  Mint NOT submitted. The example intentionally stops after simulation.'),
    );
  } catch (error: unknown) {
    console.log(chalk.yellow('Mint simulation did not succeed.'));

    console.log(chalk.gray(`  Diagnostic: ${errorMessage(error)}`));

    console.log(
      chalk.gray('The failure was handled without submitting any state-changing transaction.'),
    );
  }
  ...
  await demonstrateUnsupportedOperation(server, deployer, sacContract);
  ...
  console.log(
    chalk.cyan(
      '\nExample summary:\n' +
        ...
        `  SAC verified     : ${inspection.isStellarAssetContract ? 'YES' : 'NO'}`,
    ),
  );
