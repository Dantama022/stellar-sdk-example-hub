...
export async function run(params?: any): Promise<void> {
  ...
  try {
    ...
    const assetBInput = params?.assetB || 'USDC:GB6ZS324HT6VEEDZ6MG6CESWE7YZSY7WAJDRQSP2GZCRZ5GBND377A2F';
    ...
    console.log('\n=== Simulation Results ===');
    console.log(`Current Reserves: ${pool.reserves[0].asset} = ${reserveA}, ${pool.reserves[1].asset} = ${reserveB}`);
    console.log(`Current Total Pool Shares: ${totalShares}`);
    console.log(`Deposit Amounts: ${depositA} ${pool.reserves[0].asset}, ${depositB} ${pool.reserves[1].asset}`);
