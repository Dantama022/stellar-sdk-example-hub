export async function run(params: { input?: string; expectedType?: string } = {}) {
  ...
  if (expectedLower === 'boolean' || expectedLower === 'bool') {
    if (typeof input !== 'boolean') { isValid = false; mismatch = "Expected boolean"; }
  } else if (expectedLower === 'number' || expectedLower === 'u32' || expectedLower === 'i32') {
    if (typeof input !== 'number') { isValid = false; mismatch = "Expected number"; }
  } else if (expectedLower === 'string' || expectedLower === 'symbol') {
    if (typeof input !== 'string') { isValid = false; mismatch = "Expected string"; }
  } else if (expectedLower === 'address') {
    if (typeof input !== 'string' || (!input.startsWith('G') && !input.startsWith('C')) || input.length !== 56) {
      isValid = false; mismatch = "Expected a valid Stellar 56-character address";
    }
  } else if (expectedLower === 'vector' || expectedLower === 'array') {
    if (!Array.isArray(input)) { isValid = false; mismatch = "Expected array/vector"; }
  }
