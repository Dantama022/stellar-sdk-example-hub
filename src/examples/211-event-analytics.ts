...
export async function run(params: { startLedger?: string; endLedger?: string; contractId?: string } = {}) {
  ...
  Object.entries(byTopic).sort((a, b) => b[1] - a[1]).slice(0, 5).forEach(([t, c]) => console.log(`  ${t}: ${c}`));

  console.log(`\n--- Contracts Interacted ---`);
  Object.entries(byContract).sort((a, b) => b[1] - a[1]).slice(0, 5).forEach(([c, count]) => console.log(`  ${c}: ${count}`));
