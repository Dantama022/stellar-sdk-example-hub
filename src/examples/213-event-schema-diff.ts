...
export async function run(params: { oldSchema?: string; newSchema?: string } = {}) {
  ...
  const added = newSchema.events.filter((n: any) => !oldSchema.events.find((o: any) => o.id === n.id));
  const removed = oldSchema.events.filter((o: any) => !newSchema.events.find((n: any) => n.id === o.id));
