...
export async function run(): Promise<void> {
  ...
  const tx = new TransactionBuilder(sponsorAccount, { fee: '100', networkPassphrase: Networks.TESTNET })
  ...
  console.log(`Sponsored Data (sponsored-config): ${sponsoredAcc.data_attr['sponsored-config'] ? 'Present' : 'Missing'}`);
