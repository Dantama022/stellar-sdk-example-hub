...
export async function run(): Promise<void> {
  ...
  console.log(
    'Strict Send ensures the exact source amount is spent, while the received amount varies based on the path.',
  );
  ...
  const paths = await server
    .strictSendPaths(Asset.native(), exactSourceAmount, [customAsset])
    .call();
  ...
  console.log(
    ` - Destination Amount (Expected Receive): ${pathRecord.destination_amount} PATHASSET`,
  );
  ...
  const strictSendTx = new TransactionBuilder(sourceAcc, {
    fee: '100',
